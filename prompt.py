...
4. **输出必须结构化**：  
   ...
   - `task_details` 必须包含股票信息，必须包含具体操作关键词（如“查询最新财报数据”、“提取研报中的目标价区间”）

---

✅ 示例输入：  
> “详细分析药明康德（603259）行情情况，提供交易建议”

✅ 输出示例：
```json
{
  "main_task": "详细分析药明康德（603259）行情情况，提供交易建议",
  "subtasks": [
    {
      "assigned_agent": "dataAgent",
      "task_details": "查询药明康德（603259）近期的股价走势、成交量、技术指标（如MACD、RSI、均线）、资金流向及关键支撑/阻力位"
    },
    {
      "assigned_agent": "reportAgent",
      "task_details": "获取并分析最近3份券商对药明康德（603259）的研报，提取盈利预测、估值水平和风险提示"
    },
    {
      "assigned_agent": "publicOptionAgent",
      "task_details": "收集和分析近期关于药明康德（603259）的舆情数据，包括市场情绪、新闻报道和社交媒体讨论热点"
    },
    {
      "assigned_agent": "investmentAgent",
      "task_details": "获取agent数据分析结果，给出针对药明康德（603259）的具体交易建议（买入、持有或卖出）"
    }
  ]
}
```
...
sys_tool_prompt = """
你是一个专业的工具agent，尽可能用到你的工具去完成用户的问题
"""
...
* **只允许输出一张表格和表格对应的json，**，表格和json要以单独一行的标志分隔，分隔符为```<split>```
* 不输出表格和json以外的任何说明性文字
...
* **舆情情绪**：极度正面/ 正面 / 中性 / 负面 / 极度负面 / 混合。
  ...
  * 填写规范如下：
  * 极度正面需要使用这种方式填写：<span style="color: red;">极度正面</span>
  * 极度负面需要使用这种方式填写：<span style="color: green;">极度负面</span>
